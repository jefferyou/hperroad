...
class MultiCityExperimentRunner:
    ...
    def run_training(self, city, dataset_code, seed=0):
        ...
        print(f"Expected Experiment ID: {exp_id}")
        print(f"Using GPU: {self.args.train_gpu}")
        print(f"Note: Actual exp_id will be auto-generated by VecCity pipeline")
        ...
        # 查找最新生成的experiment ID
        cache_dir = os.path.join(self.veccity_path, 'veccity', 'cache')
        if os.path.exists(cache_dir):
            # 查找最新的exp_id（包含数据集代码）
            exp_dirs = [d for d in os.listdir(cache_dir) if d.startswith(f'hrnr_hyp_{dataset_code}')]
            if exp_dirs:
                # 按修改时间排序，获取最新的
                exp_dirs.sort(key=lambda x: os.path.getmtime(os.path.join(cache_dir, x)), reverse=True)
                exp_id = exp_dirs[0]
                print(f"✓ Detected experiment ID: {exp_id}")
            else:
                print(f"⚠ Warning: Could not find experiment directory for {dataset_code}")
